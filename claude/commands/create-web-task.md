...
- **인자 있음**: 개발과제 문서를 분석하여 Web 작업 필요여부 판단 및 작업 생성
...
```bash
command gh api repos/tpcint/likey-web/pulls/{PR번호} -X PATCH -f title="{기존제목} (LK-{ID})"
```
...
개발과제 문서를 분석하여 Web 코드베이스 분석 후 작업 필요여부 판단 및 작업 생성.
...
```
- 요구사항 파악
- 핵심 키워드, 기능명, 페이지명 등 추출

### B-2. Web 코드베이스 분석 (Nuxt.js)
- **프로젝트 경로**: `/Users/muzi/projects/likey-web`
```bash
