...
**중요: deploy 관련 checks만 확인. 다른 checks(lint, test, codereview 등)는 무시.**
...
deploy 관련 check 필터링 기준 (name에 아래 키워드 포함 여부로 판단):
- `deploy` 또는 `preview` (대소문자 무시)
- 예: `deploy-preview (preview-beta) / Deploy ccktuc (beta)`, `preview-action`
- `check`, `Vitest`, `TSC`, `Lint`, `l10n`, `CodeRabbit`, `codex`, `approve`, `skip-check`, `checklist` 등은 **무시**

완료 판단:
- deploy 관련 checks가 모두 `SUCCESS`면 배포 완료
- 하나라도 `FAILURE`면 배포 실패로 처리
- `SKIPPED`인 deploy check는 무시 (예: `remove-label`, `delete-preview`)
- deploy 관련 checks가 아직 나타나지 않으면 계속 대기
