...
| 항목 | 검사 내용 | 심각도 |
|------|----------|--------|
| 수동 재구현 | 프레임워크가 제공하는 기능을 수동으로 재구현할 때, 실제 프레임워크 동작(인코딩, 직렬화, lifecycle 등)과 일치하는지 확인 | 🟠 Major |
| 경계값 검증 | 조건 분기에서 빈 문자열(`""`), `null`, `undefined`, `0`, 빈 배열(`[]`) 등 경계값이 의도대로 처리되는지 확인 | 🟠 Major |
