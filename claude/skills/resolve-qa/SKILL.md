...
### 1-3단계: 실제 환경 로그 진단 (선택적)

QA 이슈가 API 에러(400, 500 등)와 관련된 경우, 코드를 보기 전에 **실제 환경에서 무슨 일이 일어나고 있는지** 먼저 파악한다.

**`/cloud-logs` 스킬을 활용하여 다음을 확인:**

1. 해당 preview 환경에 에러가 실제로 발생하고 있는지
2. 에러의 빈도와 패턴 (특정 엔드포인트 집중, 시간대 등)
3. trace ID로 에러의 정확한 원인 (ValidateError, 비즈니스 로직 에러 등)

```
Skill: cloud-logs
args: (QA 이슈에서 파악한 정보에 맞게 조합)
```

예시:
- QA 이슈가 룰렛 수정 시 400 에러 → `/cloud-logs 400 /api/studio/roulette`
- QA 이슈가 서버 에러 → `/cloud-logs 5xx`
- 특정 사용자에서만 재현 → `/cloud-logs {uid}`

> **이 단계는 선택적이다.** Preview가 배포되어 있지 않거나, QA 이슈가 로그와 무관한 UI 문제인 경우 스킵한다.
> 로그에서 충분한 정보를 얻지 못해도 코드 분석으로 계속 진행한다.
